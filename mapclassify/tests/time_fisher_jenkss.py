...
def test_fisher_jenks_means(N, HAS_NUMBA):
    ...
    if HAS_NUMBA:
        func = mapclassify.classifiers._fisher_jenks_means(
            np.sort(data).astype("f8"), classes=k
        )
    else:
        func = mapclassify.classifiers._fisher_jenks_means_without_numpy(
            sorted(data), classes=k
        )
...
def compare_times(test_runner, descriptions, title):
    ...
    for HAS_NUMBA, description in zip([False, True], descriptions):
        ...
        for N in data_sizes:
            t = timeit.timeit(
                functools.partial(test_runner, N=N, HAS_NUMBA=HAS_NUMBA),
                number=number_tests,
            )

            print(
                f"Time: {t:.3f} seconds, data points: {N} {description}, {number_tests=}"
            )
